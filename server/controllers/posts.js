...
export const getPostsBySearch = async(req, res) =>{
    ...
    try {
        ...
        const posts = await Post.find({ $or: [ {location},{title}, {location: {$in: locations.split(',')}}]})

        res.json({data: posts});
    } catch (error) {
        res.status(404).json({error, msg: 'something wrong in controller/posts/getpostsbysearch'})
    }
}
